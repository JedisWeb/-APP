...
public class ConditionListViewAdapter extends ArrayAdapter<ConditionListViewArray> {
     ...
     */
    public ConditionListViewAdapter(Context context, int resource, List<ConditionListViewArray> objects) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ConditionListViewArray imageListArray = getItem(position); //得到集合中指定位置的一组数据，并且实例化
        View view = LayoutInflater.from(getContext()).inflate(recourceId, parent, false); //用布局裁剪器(又叫布局膨胀器)，将导入的布局裁剪并且放入到当前布局中
        TextView num = view.findViewById(R.id.condition_layout_listview_num_textView); //从裁剪好的布局里获取TextView布局Id
        EditText questionEditText = view.findViewById(R.id.condition_layout_listview_question_editText);
        EditText answerEditText = view.findViewById(R.id.condition_layout_listview_answer_editText);
        num.setText(imageListArray.getNum());//将当前一组imageListArray类中的TextView内容导入到TextView布局中
        questionEditText.setHint("问题  如:姓名、卡号、颜色、形状等特征");
        answerEditText.setHint("答案");
