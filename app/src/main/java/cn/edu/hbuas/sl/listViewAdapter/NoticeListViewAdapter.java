...
public class NoticeListViewAdapter extends ArrayAdapter<NoticeListViewArray> {
     ...
     */
    public NoticeListViewAdapter(Context context, int resource, List<NoticeListViewArray> objects) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        NoticeListViewArray imageListArray = getItem(position); //得到集合中指定位置的一组数据，并且实例化
        View view = LayoutInflater.from(getContext()).inflate(recourceId, parent, false); //用布局裁剪器(又叫布局膨胀器)，将导入的布局裁剪并且放入到当前布局中
        TextView textView = view.findViewById(R.id.notice_layout_listview_text_textView); //从裁剪好的布局里获取TextView布局Id
        SwitchButton switchButton = view.findViewById(R.id.notice_layout_listview_switch_btn);
